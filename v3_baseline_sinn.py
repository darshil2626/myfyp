...
class sinn:
    """
    Time-conditioned SINN baseline (no Fourier features, no source term).

    Homogeneous elliptic PDE in latent space: div(A grad ell) = 0
    with time-aware encoder using raw (t, y, x) coordinates.
    Sequential 80/20 train/test split, independent per-timestep reconstruction.
    """
    ...
    def compute_pde_loss(self, patch_center_spatial_y_index, patch_center_spatial_x_index,
        ...
        for t_n in patch_time_indices_np:
            ...
            # Vectorised neighbour-to-boundary mapping. For each flat
            # (rid, nly, nlx) entry we look up bj = b_lookup[nly, nlx];
            # bj >= 0 means this neighbour is present at this timestep.
            if nbr_rid_arr.size > 0:
                ...
                if keep.any():
                    ...
                    # Build incidence M_t (n_int, n_b_t): how many boundary
                    # neighbours each interior row has at each boundary index.
                    M_t = np.zeros((n_int, n_b_t), dtype=np.float32)
                    np.add.at(M_t, (rid_keep, bj_keep), 1.0)
                    M_t_tf = tf.constant(M_t, dtype=tf.float32)
                    # rhs (n_int, r) = (M_t @ b_lat_t) @ A^T (A symmetric)
    ...
    def reconstruct_field_at_timestep(self, t_index, use_fast_solver=False,
        ...
        # RHS: A @ sum(boundary latents for each interior neighbour)
...
# Use run_v3_baseline.py as the entry point — it reads from config.py
# and handles the full 3-year dataset correctly.
if __name__ == "__main__":
    raise SystemExit(
        "Do not run v3_baseline_sinn.py directly.\n"
        "Use:  python run_v3_baseline.py\n"
    )
