...
Methodology framing:
  - 2022 (La Niña)  → training
  - 2023 (El Niño)  → test (dissimilar ENSO regime)
  - 2024 (Neutral)  → test (similar ENSO regime)

Usage:
    python plot_sst_enso_comparison.py
"""
...
# CONFIG
# ============================================================
DATA_PATH = r"c:\Users\darsh\Documents\fyp\myfyp\time_in_encoders_only\sst\sst_2022_2023_2024_combined.pkl"
SAVE_PATH = "sst_enso_phase_comparison.png"

N_DAYS = {2022: 365, 2023: 365, 2024: 366}
...
total_expected = sum(N_DAYS.values())
assert U.shape[0] == total_expected, (
    f"Expected {total_expected} timesteps, got {U.shape[0]}"
)
...
print(f"\n{'Year':<8} {'ENSO':<15} {'Min (°C)':>10} {'Max (°C)':>10} {'Range (°C)':>12}")
print(f"{'-'*60}")
enso_labels = {2022: "La Niña", 2023: "El Niño", 2024: "Neutral"}
for year in years:
    m = mean_by_year[year]
    print(f"{year:<8} {enso_labels[year]:<15} {m.min():>10.2f} {m.max():>10.2f} {m.max()-m.min():>12.2f}")
...
ax.plot(doy_common, diff_23, color=colours[2023], lw=1.5, alpha=0.85,
        label=f"2023 El Niño − 2022 La Niña  (RMSD = {stats[(2022,2023)]['rmsd']:.3f} °C)")
ax.plot(doy_common, diff_24, color=colours[2024], lw=1.5, alpha=0.85,
        label=f"2024 Neutral − 2022 La Niña  (RMSD = {stats[(2022,2024)]['rmsd']:.3f} °C)")
...
# --- Panel 3: Spatial variability across phases ---
ax = axes[2]
for year in years:
