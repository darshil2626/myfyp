...
class sinn:
    ...
    Extension of the time-conditioned SINN (v3) with an inhomogeneous
    elliptic PDE in latent space:

        div(A grad ell) = s(x, y)

    where s is produced by a learned source network G operating on the
    previous timestep's latent field. This preserves the elliptic
    well-posedness while allowing the interior latent field to sustain
    structure beyond what boundary data alone can produce.

    Training uses curriculum learning: starts with teacher forcing
    (source from interior encoder) and anneals toward autoregressive
    (source from model's own previous output).

    Inference is fully autoregressive and sequential:
      t=0  -> standard SINN (s=0)
      t>0  -> source from previous step's latent field
    """
    ...
    def build_models(self, num_latentdim, num_units, num_layers, dropout, l2_reg, lr, n_past_steps=0):
        ...
        # --- Source network G ---
        # Input: 3 coords + k*k window of r-dim latent values + k*k mask
        kk = k * k
        self.source_input_dim = 3 + kk * self.num_latentdim + kk
        ...
        # --- Source magnitude cap ---
        # The source contribution is hard-capped at this fraction of
        # the boundary RHS norm. This ensures boundary data remains
        # the primary signal while giving the source network full
        # gradients within the allowed envelope.
        self.source_max_ratio = 0.3
        ...
        print(f"  Source max ratio: {self.source_max_ratio}")
    ...
    def _stack_mask_patch_features_from_idx(self, idx_tyx, radius=None):
        ...
        if idx_tyx.size == 0:
            return np.zeros((0, 3 + 2 * kk * n_time_slices), dtype=np.float32)
        ...
        for i in range(N):
            ti, yi, xi = int(t[i]), int(y[i]), int(x[i])
                ...
                for dy in range(-r, r + 1):
                    ...
                    for dx in range(-r, r + 1):
                        ...
                        if 0 <= yy < self.ny and 0 <= xx < self.nx:
                            u_win[i, off + ptr] = float(self.U[t_slice, yy, xx])
                            m_win[i, off + ptr] = 1.0
                        ptr += 1
    ...
    def _build_source_features(self, prev_latent_np, interior_local_yx,
        ...
        """
        Build source network inputs from the previous timestep's latent field.

        prev_latent_np: (N_interior, r) ndarray of latent values on patch interior
        interior_local_yx: (N_interior, 2) local coords within patch
        Returns: (N_interior, 3 + k*k*r + k*k) ndarray
        """
        r = self.mask_radius
        k = 2 * r + 1
        kk = k * k
        ...
        # Spatial lookup: local (y,x) -> row in prev_latent
        lookup = -np.ones((height, width), dtype=np.int32)
        for row_id, (ly, lx) in enumerate(interior_local_yx):
            lookup[ly, lx] = row_id

        latent_win = np.zeros((N, kk * rdim), dtype=np.float32)
        mask_win = np.zeros((N, kk), dtype=np.float32)

        for i, (ly, lx) in enumerate(interior_local_yx):
            ptr = 0
            for dy in range(-r, r + 1):
                for dx in range(-r, r + 1):
                    ny, nx = ly + dy, lx + dx
                    if 0 <= ny < height and 0 <= nx < width:
                        row = lookup[ny, nx]
                        if row >= 0:
                            latent_win[i, ptr*rdim:(ptr+1)*rdim] = prev_latent_np[row]
                            mask_win[i, ptr] = 1.0
                    ptr += 1

        t_norm = np.full((N, 1), t_index / self.nt_norm, dtype=np.float32)
        y_norm = ((interior_local_yx[:, 0] + y_min).astype(np.float32) / self.ny_norm).reshape(-1, 1)
        x_norm = ((interior_local_yx[:, 1] + x_min).astype(np.float32) / self.nx_norm).reshape(-1, 1)

        return np.concatenate([t_norm, y_norm, x_norm, latent_win, mask_win], axis=1).astype(np.float32)

    def _build_source_features_global(self, prev_latent_np, interior_indices,
                                       interior_row_map, t_index):
        """
        Source features for full-domain inference (not patch-level).

        prev_latent_np: (N_interior, r) latent field on full interior grid
        interior_indices: (N_interior, 2) global (y,x) coords
        interior_row_map: (ny, nx) -> row index or -1
        """
        r = self.mask_radius
        k = 2 * r + 1
        kk = k * k
        ...
        for i, (yi, xi) in enumerate(interior_indices):
            ptr = 0
            for dy in range(-r, r + 1):
                for dx in range(-r, r + 1):
                    nyy, nxx = int(yi + dy), int(xi + dx)
                    if 0 <= nyy < self.ny and 0 <= nxx < self.nx:
                        row = interior_row_map[nyy, nxx]
                        if row >= 0:
                            latent_win[i, ptr*rdim:(ptr+1)*rdim] = prev_latent_np[row]
                            mask_win[i, ptr] = 1.0
                    ptr += 1
        ...
        t_norm = np.full((N, 1), t_index / self.nt_norm, dtype=np.float32)
        y_norm = (interior_indices[:, 0].astype(np.float32) / self.ny_norm).reshape(-1, 1)
        x_norm = (interior_indices[:, 1].astype(np.float32) / self.nx_norm).reshape(-1, 1)
    ...
    def compute_pde_loss(self, patch_center_spatial_y_index, patch_center_spatial_x_index,
        ...
        The source is hard-capped at self.source_max_ratio of the boundary
        RHS norm to ensure boundary data remains the primary signal.
        """
        ...
        for t_n in patch_time_indices_np:
            ...
            if prev_latent is not None:
                ...
                # Add CAPPED source to RHS — hard cap keeps boundary dominant
                src_vec = tf.concat(
                    [tf.reshape(src_vals[i, :], (self.num_latentdim, 1)) for i in range(n_int)],
                    axis=0
                )
                # Normalise: source can't exceed max_ratio of boundary RHS
                bnd_norm = tf.norm(rhs_vec) + 1e-8
                src_norm = tf.norm(src_vec) + 1e-8
                cap = tf.minimum(1.0, self.source_max_ratio * bnd_norm / src_norm)
                rhs_vec = rhs_vec + cap * src_vec
    ...
    def _reconstruct_single_timestep(self, t_index, prev_latent_interior=None,
        ...
        def stack_observed_only(idx_tyx):
            ...
            feats[:, 0] = idx_tyx[:, 0].astype(np.float32) / max(self.nt - 1, 1)
            feats[:, 1] = idx_tyx[:, 1].astype(np.float32) / max(self.ny - 1, 1)
            feats[:, 2] = idx_tyx[:, 2].astype(np.float32) / max(self.nx - 1, 1)
            u_off = 3
            m_off = 3 + ws * nts
            for i in range(N):
                ti, yi, xi = idx_tyx[i]
                for p in range(nts):
                    ...
                    for dy in range(-r, r + 1):
                        ...
                        for dx in range(-r, r + 1):
                            xx = xi + dx
                            if 0 <= yy < self.ny and 0 <= xx < self.nx and obs_mask[yy, xx]:
                                feats[i, u_off + so + ptr] = float(self.U[ts, yy, xx])
        ...
        for rid in range(num_interior):
            for bidx in boundary_contributions[rid]:
                rhs[rid, :] += A_np @ bnd_latents[bidx, :].astype(np.float64)
        ...
        if prev_latent_interior is not None:
            ...
            # Hard cap: source can't exceed max_ratio of boundary RHS
            bnd_norm = np.linalg.norm(rhs) + 1e-8
            src_norm = np.linalg.norm(src_vals) + 1e-8
            cap = min(1.0, self.source_max_ratio * bnd_norm / src_norm)
            rhs = rhs + cap * src_vals
...
if __name__ == "__main__":
    ...
    solver.plot_training_history(loss_history, save_path="training_loss_v4.png", show=True)
    ...
    solver.plot_test_mae_over_time(test_summary["per_step"],
                                   save_path="test_mae_over_time.png")
    ...
    # Save results
    solver.save_test_results(save_path="test_results_v4.pkl",
