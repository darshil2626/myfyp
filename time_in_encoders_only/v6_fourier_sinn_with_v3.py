...
class sinn:
    """
    Time-conditioned SINN with multi-scale Fourier feature embeddings.

    Homogeneous elliptic PDE in latent space: div(A grad ell) = 0
    with time-aware encoder using Fourier coordinate embeddings.

    No source term — each timestep is reconstructed independently
    from boundary observations. This is the baseline for evaluating
    whether Fourier features alone can reduce oversmoothing.
    """
    ...
    def _init_fourier_B(self):
        ...
        self.fourier_B = np.concatenate(blocks, axis=0)
        self.coord_embed_dim = 2 * self.n_fourier_frequencies

    def _fourier_embed_coords(self, t_norm, y_norm, x_norm):
        N = len(t_norm)
        z = np.stack([t_norm, y_norm, x_norm], axis=1).astype(np.float32)
        proj = 2.0 * np.pi * (z @ self.fourier_B.T)
    ...
    def build_models(self, num_latentdim, num_units, num_layers, dropout, l2_reg, lr, n_past_steps=0):
        ...
        print(f"  Fourier: {self.n_fourier_frequencies} freq -> {self.coord_embed_dim} coord features")
...
if __name__ == "__main__":
    ...
    solver.plot_training_history(loss_history, save_path="training_loss_v3f.png", show=True)
    ...
    solver.plot_test_mae_over_time(test_summary["per_step"], save_path="test_mae_over_time_v3f.png")
    ...
    solver.save_test_results(save_path="test_results_v3f.pkl",
